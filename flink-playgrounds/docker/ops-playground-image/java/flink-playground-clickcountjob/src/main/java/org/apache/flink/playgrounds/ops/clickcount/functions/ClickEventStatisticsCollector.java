...
public class ClickEventStatisticsCollector
	...
	private ValueState<Long> windowsCount;
	...
	public void process(
		...
		windowsCount.update(windowsCount.value() + 1);

		out.collect(new ClickEventStatistics(new Date(context.window().getStart()), new Date(context.window().getEnd()), page, count, windowsCount.value()));
	}

	@Override
	public void open(Configuration parameters) throws Exception {
		ValueStateDescriptor<Long> windowsCount = new ValueStateDescriptor<>("windows count", Long.class, 0L);
		this.windowsCount = getRuntimeContext().getState(windowsCount);
	}
}
